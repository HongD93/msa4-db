...
-- LIKE 절 : 문자열의 내용을 조회(속도가 느림)
-- % : 글자수와 상관없이 조회할 경우 사용
-- 우가 이름 마지막에 포함된 사원 조회
SELECT
	*
FROM employees
WHERE
...
-- 이름이 세글자인 사원중 가운데 우가 들어가는 사원 조회
SELECT
	*
FROM employees
WHERE
...
-- 현재 받고있는 급여 중 가장 많이 받는 급여와, 가장 적게 받는 급여를 출력해주세요.
SELECT
	MAX(salary)
	,MIN(salary)
FROM salaries
